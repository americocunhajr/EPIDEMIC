...
for init = 1:1:14
    ...
    days = 0:1:n-1;
    fig  = semilogy(days,tot_deaths_X,'DisplayName',[country,'  ',num2str(max(tot_deaths)),' deaths'],'color',color,'LineWidth',1.25);
    hold on;
    text (n-1,tot_deaths_X(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot total cases by time (day zero defined by cases)
    figure (2);
    n    = max(max(size(tot_cases_X)));
    days = 0:1:n-1;
    fig  = semilogy(days,tot_cases_X,'DisplayName',[country,'  ',num2str(max(tot_cases)),' cases'],'color',color,'LineWidth',1.25);
    hold on;
    text (n-1,tot_cases_X(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot new deaths vs total deaths
    figure (3);
    n    = max(max(size(tot_deaths)));
    fig  = loglog(tot_deaths,new_deaths7,'DisplayName',[country,'  ',num2str(max(tot_deaths)),' deaths'],'color',color,'LineWidth',1.25);
    hold on;
    text (tot_deaths(n,1),new_deaths7(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot new cases vs total cases
    figure (4);
    n    = max(max(size(tot_cases)));
    fig  = loglog(tot_cases,new_cases7,'DisplayName',[country,'  ',num2str(max(tot_cases)),' cases'],'color',color,'LineWidth',1.25);
    hold on;
    text (tot_cases(n,1),new_cases7(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot new deaths by time (zero day defined by deaths)
    figure (5);
    n    = max(max(size(new_deaths_X)));
    days = 0:1:n-1;
    fig  = semilogy(days,new_deaths7_X,'DisplayName',[country,'  ',num2str(max(tot_deaths)),' deaths'],'color',color,'LineWidth',1.25);
    hold on;
    text (n-1,new_deaths7_X(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot new cases by time (day zero defined by cases)
    figure (6);
    n    = max(max(size(new_cases_X)));
    days = 0:1:n-1;
    fig  = semilogy(days,new_cases7_X,'DisplayName',[country,'  ',num2str(max(tot_cases)),' cases'],'color',color,'LineWidth',1.25);
    hold on;
    text (n-1,new_cases7_X(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot total deaths / million by time (zero day defined by deaths / million)
    figure (7);
    n    = max(max(size(tot_deaths_pmX)));
    days = 0:1:n-1;
    fig  = semilogy(days,tot_deaths_pmX,'DisplayName',[country,'  ',num2str(max(tot_deaths)),' deaths'],'color',color,'LineWidth',1.25);
    hold on;
    text (n-1,tot_deaths_pmX(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot total cases / million by time (day zero defined by cases / million)
    figure (8);
    n    = max(max(size(tot_cases_pmX)));
    days = 0:1:n-1;
    fig  = semilogy(days,tot_cases_pmX,'DisplayName',[country,'  ',num2str(max(tot_cases)),' cases'],'color',color,'LineWidth',1.25);
    hold on;
    text (n-1,tot_cases_pmX(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot new deaths vs total deaths (per million inhab.)
    figure (9);
    n    = max(max(size(tot_deaths_pm)));
    fig  = loglog(tot_deaths_pm,new_deaths7_pm,'DisplayName',[country,'  ',num2str(max(tot_deaths)),' deaths'],'color',color,'LineWidth',1.25);
    hold on;
    text (tot_deaths_pm(n,1),new_deaths7_pm(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot new cases vs total cases (per million inhab.)
    figure (10);
    n    = max(max(size(tot_cases_pm)));
    fig  = loglog(tot_cases_pm,new_cases7_pm,'DisplayName',[country,'  ',num2str(max(tot_cases)),' cases'],'color',color,'LineWidth',1.25);
    hold on;
    text (tot_cases_pm(n,1),new_cases7_pm(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot new deaths / million by time (zero day defined by deaths / million)
    figure (11);
    n    = max(max(size(new_deaths_pmX)));
    days = 0:1:n-1;
    fig  = semilogy(days,new_deaths7_pmX,'DisplayName',[country,'  ',num2str(max(tot_deaths)),' deaths'],'color',color,'LineWidth',1.25);
    hold on;
    text (n-1,new_deaths7_pmX(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');

    % Plot new cases / million by time (day zero defined by cases / million)
    figure (12);
    n    = max(max(size(new_cases_pmX)));
    days = 0:1:n-1;
    fig  = semilogy(days,new_cases7_pmX,'DisplayName',[country,'  ',num2str(max(tot_cases)),' cases'],'color',color,'LineWidth',1.25);
    hold on;
    text (n-1,new_cases7_pmX(n,1),[' ',country],'FontSize',font_location,'color',color,'Clipping','on');
